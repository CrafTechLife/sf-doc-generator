    ...
    if (customFields.length > 0) {
      ...
      for (let i = 0; i < fieldFullNames.length; i += batchSize) {
        ...
        try {
          ...
          // 一部のフィールド（地理位置情報の緯度・経度など）は個別取得できないことがある
          // ただし、CustomObject全体から既に取得済みの場合が多いため、実質的に問題ない
          console.log(
            `ℹ️  一部のカスタム項目の個別メタデータ取得をスキップしました（想定内の動作）`
          );
          console.log(`   理由: ${error.message}`);
          console.log(`   対象: ${batch.join(", ")}`);
          console.log(
            `   ※CustomObject全体から既に取得済みの場合は正常に出力されます\n`
          );
        }
      }
